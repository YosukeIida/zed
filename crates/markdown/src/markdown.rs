...
use gpui::{
    ...
    canvas, img, point, quad, relative, size,
};
...
impl RenderedLine {
    /// Painted before the glyphs so the text renders on top of the chips. Reuses
    /// [`LineGeometry::highlight_bounds`] (shared with the math-flow layout kind) the same way
    /// [`Self::paint_highlights`] does, since `code_chips` ranges are already in rendered indices.
    fn paint_code_chips(&self, window: &mut Window) {
        for (rendered_range, color) in &self.code_chips {
            for bounds in self
                .layout
                .highlight_bounds(rendered_range.clone(), self.text_align)
            {
